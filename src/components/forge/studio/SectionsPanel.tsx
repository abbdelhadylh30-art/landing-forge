  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected ? "border-violet-500/60 bg-violet-500/10" : "border-zinc-800/80 bg-zinc-900/40 hover:border-zinc-700",
        section.hidden && "opacity-45",
        isDragging && "z-50 shadow-lg shadow-black/50 ring-1 ring-violet-500/50"
      )}
    >
      <button
        type="button"
      ...
      <div className="flex shrink-0 items-center opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
        ...
        <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-500 hover:text-zinc-200" onClick={() => duplicateSection(section.id)} aria-label="Duplicate section" title="Duplicate">
          <Copy className="h-3 w-3" />
        </Button>
        <Button variant="ghost" size="icon" className="h-6 w-6 text-zinc-500 hover:text-rose-300" onClick={() => removeSection(section.id)} aria-label="Delete section" title="Delete">
