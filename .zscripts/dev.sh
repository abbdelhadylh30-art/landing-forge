...
# NOTE: the dev-server watchdog (.zscripts/dev-warmup-loop.sh) is spawned by
# the analytics-live relay (see mini-services/analytics-live/index.ts) — the
# relay is started by start_mini_services above and is a persistent home for
# it (children it spawns survive). Do NOT start it directly here as well.
